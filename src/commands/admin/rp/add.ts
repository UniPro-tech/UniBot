...
export const data = new SlashCommandSubcommandBuilder()
  ...
  .addRoleOption((option) => option.setName('role1').setDescription('追加するロールを選択してください(任意)'))
  .addRoleOption((option) => option.setName('role2').setDescription('追加するロールを選択してください(任意)'))
  .addRoleOption((option) => option.setName('role3').setDescription('追加するロールを選択してください(任意)'))
  .addRoleOption((option) => option.setName('role4').setDescription('追加するロールを選択してください(任意)'))
  .addRoleOption((option) => option.setName('role5').setDescription('追加するロールを選択してください(任意)'))
  .addRoleOption((option) => option.setName('role6').setDescription('追加するロールを選択してください(任意)'));
...
export const execute = async (interaction: ChatInputCommandInteraction) => {
  ...
  if (!botMember) {
     await interaction.reply({ content: 'ボットのメンバー情報が取得できません', flags: MessageFlags.Ephemeral });
     return;
  }
  ...
  const send = new EmbedBuilder().setColor('#4CAF50').setTitle(panelTitle).setDescription(description).setTimestamp();
