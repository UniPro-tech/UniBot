...
export const data = new SlashCommandSubcommandBuilder()
  ...
  .addStringOption((option) =>
     option.setName('title').setDescription('役職パネルの名前を設定してください(任意、デフォルトでは役職パネル)')
  )
  .addRoleOption((option) => option.setName('role1').setDescription('付与するロールを選択してください(任意)'))
  .addRoleOption((option) => option.setName('role2').setDescription('付与するロールを選択してください(任意)'));
