...
func Remove(ctx *internal.BotContext) func(data discord.SlashCommandInteractionData, e *handler.CommandEvent) error {
	return func(data discord.SlashCommandInteractionData, e *handler.CommandEvent) error {
		...
		var err error
		if isAdmin {
			entries, err = query.TtsDictionary.Where(query.TtsDictionary.GuildID.Eq(int64(*e.GuildID()))).Find()
		} else {
			entries, err = query.TtsDictionary.Where(query.TtsDictionary.GuildID.Eq(int64(*e.GuildID())), query.TtsDictionary.UserID.Eq(int64(e.User().ID))).Find()
		}
		...
		for idx, entry := range displayEntries {
			options[idx] = discord.StringSelectMenuOption{
				Label:       entry.Word,
				Value:       fmt.Sprintf("%s", entry.ID),
				Description: entry.Yomi,
			}
		}
